...
export default function PipelineView({ clubs, statuses, onUpdateClub, onSelect, selectedSeason }) {
    ...
    // Detectar si estamos en vista móvil para desactivar el drag nativo y permitir scroll táctil
    useEffect(() => {
        const checkIfMobile = () => {
        ...
        <div className="flex-1 bg-zinc-50/50 dark:bg-zinc-950 p-4 md:p-6 flex flex-col h-full overflow-hidden">
            <div className="mb-4 md:mb-6 flex-shrink-0">
                ...
                <p className="text-zinc-500 dark:text-zinc-400 text-xs md:text-sm">
                    ...
                    <span className="md:hidden">Usa los botones de cada tarjeta para cambiarla de fase. Desliza hacia abajo para ver más.</span>
                </p>
            </div>

            {/* Contenedor del Kanban adaptado para móvil con h-full */}
            <div className="flex-1 overflow-x-auto overflow-y-hidden flex gap-4 pb-6 md:pb-4 select-none snap-x snap-mandatory h-full">
                {statuses.map(status => {
                    ...
                    return (
                        <div 
                            ...
                            // Añadido h-full para forzar al contenedor a tener límite de altura y permitir scroll interior
                            className="bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl flex flex-col w-[85vw] sm:w-[320px] min-w-[85vw] sm:min-w-[320px] h-full max-h-full snap-center"
                        >
                            <div className="p-3 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-white dark:bg-zinc-900 rounded-t-xl sticky top-0 flex-shrink-0 z-10">
                            ...
                            <div className="p-3 flex-1 overflow-y-auto flex flex-col gap-3 custom-scrollbar pb-24 md:pb-3 touch-pan-y overscroll-contain">
                                {statusClubs.map(club => (
                                    <div
                                        key={club.id}
                                        // Desactivamos draggable en móvil porque bloquea el evento de scroll vertical
                                        draggable={!isMobile}
                                        onDragStart={(e) => !isMobile && handleDragStart(e, club.id)}
                                        onDragEnd={handleDragEnd}
                                        onClick={(e) => {
                                            // Prevenir selección si tocamos un botón interactivo
                                                ...
                                                return (
                                                    <>
                                                        <button 
                                                            ...
                                                            className="flex-1 bg-zinc-100 dark:bg-zinc-800 disabled:opacity-30 text-zinc-600 dark:text-zinc-400 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 flex items-center justify-center gap-1 cursor-pointer"
                                                        >
